...
def createScene(pipeline):
    ...
    mountainsNode.transform = tr.matmul([tr.translate(0, -0.2, 0), tr.scale(1, 1, 1)])
    mountainsNode.childs = [mountain1Node, mountain2Node, mountain3Node]

     # Nodo que agrupa a las montañas, posicionado
    mountains1Node = sg.SceneGraphNode("cordillera1")
    mountains1Node.transform = tr.matmul([tr.translate(0, -0.2, 0), tr.scale(1, 1, 1)])
    mountains1Node.childs = [mountainsNode]

    # Nodo que agrupa a las montañas, posicionado
    mountains2Node = sg.SceneGraphNode("cordillera2")
    mountains2Node.transform = tr.matmul([tr.translate(2, -0.2, 0), tr.scale(1, 1, 1)])
    mountains2Node.childs = [mountainsNode]

    # Nodo que agrupa los dos montones de montañas (Se que el nombre presta para confusion pero es tarde y no se me ocurre algo mejor)
    mountainsNode = sg.SceneGraphNode("mountains")
    mountainsNode.childs = [mountains1Node, mountains2Node]
    ...
    translateWindMill2Node.transform = tr.matmul([tr.translate(-0.25, 0.3, 0), tr.scale(0.7, 0.7, 0.7)])
    ...
    # Nodo de trio de molinos 1
    trioMolinos1Node = sg.SceneGraphNode("trio1")
    trioMolinos1Node.childs = [windMillGroupNode]

    # Nodo de triode molinos 2
    trioMolinos2Node = sg.SceneGraphNode("trio2")
    trioMolinos2Node.transform = tr.translate(2,0,0)
    trioMolinos2Node.childs = [windMillGroupNode]

    # Nodo que agrupa los trios
    molinosNode = sg.SceneGraphNode("molinos")
    molinosNode.childs = [trioMolinos1Node,trioMolinos2Node]
    ...
    backGroundNode.childs = [skyNode, sunNode, mountainsNode, highwayNode, molinosNode, lineNode]
